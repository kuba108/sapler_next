...
$(function() {
    ...
    $.validator.addMethod("distributor_not_equals", function(value, element, arg) {
        return arg != value;
    }, "Vyberte prosím odběrné místo ze seznamu.");

	$.extend($.validator.messages, {
        formatDIC: 'Prosím, zadejte platný DIČ.',
        formatIC: 'Prosím, zadejte platné IČO.',
...
var CookieManager = {
  ...
  check_cookie_approval: function() {
    var cappr = CookieManager.get_cookie("__cappr");
...
var Gallery = {
  ...
  init: function() {
    image_links = document.getElementsByClassName('image-link');
    for(var i = 0; i < image_links.length; i++) {
      image_links[i].onclick = function (event) {
        event.preventDefault();
        link = event.target.src ? event.target.parentNode : event.target;
        options = {index: link, event: event};
        blueimp.Gallery(image_links, options);
      }
    }
  }
